...
public class UserTableDriver {
    ...
    // Include time zone offset (xxx) since postgres timestamps with the time zone
    public static final DateTimeFormatter DATE_FORMATTER = new DateTimeFormatterBuilder()
            .appendPattern("yyyy-MM-dd HH:mm:ss")
            .optionalStart()
            .appendFraction(ChronoField.MICRO_OF_SECOND, 3, 6, true)
            .optionalEnd()
            // Offset is optional, so we can parse records without it. Postgres only offsets to the Hour level
            .appendOffset("+HH", "Z")
            .toFormatter();
    ...
    private static final List<TableColumnType> COLUMN_TYPES = List.of(
      ...
      new TableColumnType(USERNAME_COLUMN, TEXT_NOT_NULL),
      new TableColumnType(PASSWORD_COLUMN, TEXT_NOT_NULL),
      new TableColumnType(EMAIL_COLUMN, TEXT_NOT_NULL),
    ...
    // Columns used to insert/update a user
    private static final List<String> INSERT_COLUMNS = List.of(USERNAME_COLUMN, PASSWORD_COLUMN, EMAIL_COLUMN,
    ...
    /**
     * Insert a user record into the users table from a UserRecordInsert. Null values will be entered as empty string
     *
    ...
    public UserRecordResult insertUserRecord(final UserRecordInsert userRecord) {
        ...
        // values
        final Map<String, String> inputValues = INSERT_COLUMNS.stream().collect(Collectors.toMap(c -> c, c -> {
        ...
        final List<ColumnSetter> columnSetters = INSERT_COLUMNS.stream()
        ...
        return dbResultToUserRecordResult(result);
    }

    private static UserRecordResult dbResultToUserRecordResult(final Map<String, Object> result) {
        return new UserRecordResult(safeMapValueToString(result, ID_COLUMN),
          safeMapValueToString(result, USERNAME_COLUMN),
          safeMapValueToString(result, EMAIL_COLUMN),
          safeMapValueToString(result, FIRST_NAME_COLUMN),
          safeMapValueToString(result, LAST_NAME_COLUMN),
          safeMapValueToString(result, CREATED_AT_COLUMN),
          safeMapValueToString(result, UPDATED_AT_COLUMN));
    }


    /**
     * Converts the value for the given key in the map to a String.
     ...
     * @return
     */
    private static String safeMapValueToString(final Map<String, Object> map, final String key) {

        return Objects.isNull(map.get(key)) ? null : map.get(key).toString();
    ...
    public UserRecordResult updateUserRecord(final UserRecordMutate userRecord) {
        ...
        final Map<String, String> inputValues = INSERT_COLUMNS.stream()
        ...
        // Add a value for the new updated time
        inputValues.put(UPDATED_AT_COLUMN, DATE_FORMATTER.format(ZonedDateTime.now()));

        final List<ColumnSetter> columnSetters = inputValues.entrySet().stream()
          .filter(nullSafeColumn -> !StringUtils.isEmpty(nullSafeColumn.getValue()))
          .map(column -> new ColumnSetter(column.getKey(), column.getValue())).toList();
        ...
        return dbResultToUserRecordResult(updatedUserRecord);
